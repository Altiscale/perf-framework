...
# This is throwaway code(read: full of hacks) that would be
# replaced by an emr launcher in perf-framework
...
def tag_cluster(settings, logger, jobflow_id)
  instance_cmd = "ec2-describe-tags --filter \"resource-type=instance\" "\
  "--filter \"key=aws:elasticmapreduce:job-flow-id\" "\
  "--filter \"value=#{jobflow_id}\" --region us-west-2 | awk '{print $3}'"
  ...
  instance_array.each do |instance|
    tagging_cmd = "ec2-create-tags #{instance} "\
    "--tag \"User=#{user}@altiscale.com\" --tag \"Customer=Engineering\" --region us-west-2"
...
begin
  ...
  run_job_cmd = "#{run_job_cmd} -f #{settings.from_dir} "\
  "-t #{settings.to_dir}/#{jobflow_id}" unless settings.from_dir.nil? || settings.to_dir.nil?
  num = 0
